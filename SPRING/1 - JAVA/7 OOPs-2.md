...
- **cannot be instantiated** means can't be used to make any objects 
...
Then polymorphism is useless.

# Extremely Important Rule

You can always store child object in parent reference:
...
Because Animal is more general.

Dog is more specific.
...
`a.wagTail()` will NOT work.



---

## ❌ It’s NOT because “another object might come”

That’s not the reason.

The real reason is:

> The compiler only looks at the **reference type**, not the object type and in the animal class there's no wigtail method defined.
...
👉 "Does Animal class have wagTail() method?"

Answer: NO.

So it throws compile-time error.

It doesn’t care that the object is Dog.

### Polymorphism works only for **overridden methods**,  not for new methods added in child.
...
# ⭐ 14. Interfaces in Backend System Architecture 
...
Earlier we did `Animal a = new Dog();`  
Now we’re doing `PaymentGateway pg = new UPIPayment();`  
But interface isn’t a class… so how is this allowed?


# 🔹 First Truth: Interface Is Also a Type

Even though interface is not a class, it is still a **reference type** in Java.
...
> “Store a UPIPayment object, but treat it as a PaymentGateway.”

This is allowed because:

`UPIPayment implements PaymentGateway`

So UPIPayment IS-A PaymentGateway (conceptually).

Even though it says “implements” instead of “extends”, the relationship is still IS-A.


# 🔹 Compare Both Cases Side by Side
...
UPIPayment implements PaymentGateway  
UPIPayment IS-A PaymentGateway


The polymorphism mechanism is identical.

---

# 🔹 Why It Feels Odd To You

Because:

- Abstract class → has partial implementation
    
- Interface → originally had no implementation
    

But polymorphism does not care about implementation.

It only cares about:

> “Does this object promise to follow this contract?”

And when a class implements an interface, it promises:

> “I guarantee I will implement all these methods.”

So Java allows you to treat it as that interface type.
...
- Interface = rulebook / contract
    

But both create a **parent type**.

And polymorphism works on types.


# 🔹 The Real Mental Model

Polymorphism works whenever:

`Reference type  ←  Object type`

And object type is compatible (child / implementer).

It does NOT matter if parent type is:
...
All are valid reference types.

**Interface polymorphism is actually MORE powerful than abstract class polymorphism.**
...
# 1️⃣ Yes — You CAN Make Variable of Interface Type
...
Because a variable of interface type simply means:

> “This variable will hold a reference to ANY object that implements this interface.”
...
`OrderService order1 = new OrderService(new CardPayment());`

Inside OrderService:

`paymentProcessor → points to CardPayment object`

When:

`paymentProcessor.pay(amount);`

JVM sees actual object → CardPayment  
So it runs CardPayment.pay()

Second time → UPIPayment
...
> “Give me ANYTHING that knows how to pay.”

It doesn’t care HOW.

That responsibility belongs to payment classes.
...
## Interface variable DOES NOT mean:

> It stores an interface.

It stores:

> A reference to an object that implements that interface.
